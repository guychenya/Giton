...
const ExampleDetailModal: React.FC<ExampleDetailModalProps> = ({ example, content, isLoading, onClose, contentRef, onAskAssistant, onSave, repoUrl, isDarkMode = true }) => {
        ...
        <div className="flex items-start sm:items-center justify-between p-4 sm:p-5 border-b border-white/10 flex-shrink-0 gap-4">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-grow">
              <h2 id="modal-title" className={`text-xl sm:text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{example.name}</h2>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                <a
                  href={example.repoUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`flex items-center gap-1.5 text-sm hover:text-purple-400 transition-colors duration-200 group ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}
                >
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-16 space-y-6">
                        ...
                        <div className="text-center space-y-2">
                            <p className={`text-lg font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Generating Documentation</p>
                            <p className={`text-sm animate-pulse ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Analyzing {example.name}...</p>
                        </div>
